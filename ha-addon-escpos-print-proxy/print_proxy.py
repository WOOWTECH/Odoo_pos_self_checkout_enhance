...
Contract (shared with the Odoo `controllers/print_proxy.py` relay path):
    ...
    POST /print
        ...
        Body: {
          ...
          "copies": int  (optional, default 1, range 1–10),
...
def create_app():
    ...
    def do_print():
        ...
        copies = max(1, min(10, int(data.get("copies") or 1)))

        _logger.info("dispatch -> %s (paper=%dmm, copies=%d, source=%s)", printer_ip, paper, copies, ip_source)
        result = print_image(printer_ip, 9100, image_b64, paper_width=paper, copies=copies, timeout=5)
