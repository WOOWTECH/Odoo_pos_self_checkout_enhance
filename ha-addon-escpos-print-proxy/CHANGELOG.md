...
- **Print Copies support**: read `copies` (1–10) from the Odoo relay
  payload and repeat the raster image + cut N times in a single TCP
  connection. Default remains 1 (backward compatible).
- Updated bundled `escpos_min.py` driver with `copies` parameter.
