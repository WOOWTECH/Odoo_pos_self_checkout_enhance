...
class PosPrinter(models.Model):
    ...
    escpos_print_copies = fields.Integer(
        string='Print Copies',
        default=1,
        help='Number of copies to print for each job (1–10). Default is 1.',
    )
    ...
    @api.constrains('escpos_print_copies')
    def _constrains_escpos_print_copies(self):
        for record in self:
            if record.printer_type != 'network_escpos':
                continue
            if record.escpos_print_copies < 1 or record.escpos_print_copies > 10:
                raise ValidationError(_(
                    "Print Copies must be between 1 and 10."
                ))
    ...
    def _send_via_relay(self, image_b64):
        ...
        payload = {
            ...
            'copies': max(1, min(10, self.escpos_print_copies or 1)),
        }
