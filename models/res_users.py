...
class ResUsers(models.Model):
    ...
    def _get_portal_pos_configs(self):
        ...
        - The POS config must have ``module_pos_hr`` enabled (Log in with
          Employees).
        - If ``basic_employee_ids`` is empty the config is hidden from
          the portal (no one may access via the portal page).
        ...
        for cfg in configs:
            if not cfg.basic_employee_ids:
                # Empty employee list — hide from portal.
                continue
            if employee in cfg.basic_employee_ids or employee in cfg.advanced_employee_ids:
                result |= cfg
        return result
