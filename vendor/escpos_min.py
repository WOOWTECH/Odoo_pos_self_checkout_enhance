...
Public API:
    print_image(printer_ip, port, b64_jpeg, paper_width=80, copies=1, timeout=3)
...
def print_image(printer_ip, port, b64_jpeg, paper_width=80, copies=1, timeout=3):
    """Decode the JPEG receipt and print it on a network ESC/POS printer.

    :param copies: Number of copies to print (1–10). Each copy is a full
        raster image followed by a paper cut. All copies are sent in a
        single TCP connection to avoid repeated socket open/close overhead.
    ...
    copies = max(1, min(10, int(copies or 1)))
    ...
    try:
        single_page = _image_to_raster(img) + b'\n\n\n' + CMD_CUT
        payload = CMD_INIT + single_page * copies
        _send_bytes(printer_ip, port, payload, timeout)
        _logger.info(
            "ESC/POS printed %d copy(ies) to %s:%s (%dx%d)",
            copies, printer_ip, port, img.width, img.height,
        )
