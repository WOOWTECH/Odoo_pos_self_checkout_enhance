...
![入口首頁](screenshots/zh-TW/portal-my-pos-card.png)
...
1. 入口使用者登入後前往 `/my` 頁面。
2. 在「Point of Sale」區段可查看所有歷史訂單。
3. 點擊訂單可檢視訂單明細、金額、付款狀態等資訊。

**疑難排解**

- 看不到 Point of Sale 區段：確認使用者帳號已設為入口使用者並具有適當權限。
...
在聯絡人設定中為合作夥伴指派可使用的 POS 設定（店鋪），多店鋪時入口使用者可在 `/my/pos` 從所有啟用的 POS 設定中選擇店鋪。

**設定**

1. 開啟聯絡人應用程式（或前往 `/odoo/contacts`）> 選擇合作夥伴。
2. 切換至「Portal POS」分頁。
...
1. 若合作夥伴被指派單一 POS 設定，登入後直接進入 POS 收銀介面。
2. 若被指派多個 POS 設定，登入後前往 `/my/pos` 選擇要使用的店鋪，選擇後進入 POS 收銀介面。
...
![合作夥伴 Portal POS](screenshots/zh-TW/partner-portal-pos.png)

**設定**

1. 前往 POS營業點 > 配置 > 設定 > 手機自助下單及自助點餐機區段。
2. 設定「預設使用者」，指定入口使用者操作 POS 時使用的內部帳號。
